...
function brandColors(data: Brand) {
  const primary = isLikelyHexColor(data.brandPrimary) ? data.brandPrimary!.trim() : '#3B82F6'
  const secondary = isLikelyHexColor(data.brandSecondary) ? data.brandSecondary!.trim() : '#1E40AF'
...
export async function sendConfirmacionReserva(data: ConfirmacionClienteEmail) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      replyTo: data.replyTo || undefined, // ✅ Resend usa reply_to
          ...
          <body>
            ...
            <div class="content">
              ...
              <div class="divider footer">
                <p style="margin:0;" class="subtle">Este es un correo automático.</p>
